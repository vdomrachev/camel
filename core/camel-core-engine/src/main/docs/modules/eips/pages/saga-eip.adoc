...
Users can follow instructions present on the Narayana website to *startup a remote instance of the coordinator*.
...
*Both actions must be executed, or none of them*: an order placed without credit can be considered an inconsistent outcome (and a payment without an order).
...
Both services called by the _direct:buy_ route can *participate in the Saga* and declare their compensating actions.
...
The compensating action of _direct:newOrder_ is _direct:cancelOrder,_ and it's shown below:
...
There is a similar endpoint for manually compensating the Saga (_saga:compensate_).
